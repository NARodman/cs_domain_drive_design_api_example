...
public class CylinderController : ControllerBase
{
    ...
    /// </summary>
    /// <param name="request">The cylinder creation request containing radius and height.</param>
    /// <returns>
    ...
    public async Task<ActionResult<Guid>> CreateCylinder([FromBody] CreateCylinderRequest request)
    {
        ...
        if (request.Radius <= 0 || request.Height <= 0)
        {
            _logger.LogWarning(
                "CreateCylinder called with invalid dimensions: Radius={Radius}, Height={Height}",
                request.Radius, request.Height);

            return BadRequest("Radius and Height must be greater than 0.");
        }
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Cylinder created successfully with Id: {CylinderId}, Radius: {Radius}, Height: {Height}",
    ...
    public async Task<ActionResult<CylinderResponse>> GetCylinderById(Guid id)
    {
        ...
        {
            _logger.LogError(ex,
                "Unexpected error occurred while retrieving cylinder with Id: {CylinderId}",
                id);
